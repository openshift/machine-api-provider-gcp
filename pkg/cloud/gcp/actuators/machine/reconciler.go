...
const (
	...
	// the following values comes from the WMCO code, see https://github.com/openshift/windows-machine-config-operator/blob/master/pkg/secrets/secrets.go
	windowsScriptSecretName      = "windows-user-data"
	windowsScriptSecretNamespace = "openshift-machine-api"
	wmcoSecretDataKey            = "userData"
)
...
func (r *Reconciler) create() error {
	...
	// check to see if this is a windows instance
	...
	if found && osid == "Windows" {
		...
		if len(script) == 0 {
			// didn't find in provider spec, look for secret
			script, err = r.getWindowsScriptFromSecret()
			if err != nil {
				return fmt.Errorf("error getting Windows script secret data: %v", err)
			}
		}

		if len(script) > 0 {
			// found a script, we need to update the instance data that will be sent to GCP
			found := false
			for i, item := range instance.Metadata.Items {
				if item.Key == windowsScriptMetadataKey {
					instance.Metadata.Items[i].Value = &script
					found = true
					break
				}
			}
			...
			if !found {
				// if we did not find this key in the previous metadata, add it now
			...
			// we didn't find a script
			return machinecontroller.CreateMachine("cannot find Windows script in providerSpec or Secret")
		}
...
// Return the value of the `sysprep-specialize-script-ps1` key from the provider spec metadata
// or an empty string if not found.
func (r *Reconciler) getWindowsScriptFromProviderSpec() string {
	script := ""

	for _, item := range r.providerSpec.Metadata {
		if item.Key == windowsScriptMetadataKey {
			script = windows.RemovePowershellTags(*item.Value)
			break
		}
	}
...
// Return the Secret data generated by the WMCO or return an empty string and error on failure.
func (r *Reconciler) getWindowsScriptFromSecret() (string, error) {
	var scriptSecret corev1.Secret
	if err := r.coreClient.Get(context.Background(), client.ObjectKey{Namespace: windowsScriptSecretNamespace, Name: windowsScriptSecretName}, &scriptSecret); err != nil {
		if apimachineryerrors.IsNotFound(err) {
			return "", machinecontroller.InvalidMachineConfiguration("Windows script data secret %q in namespace %q not found: %v", windowsScriptSecretName, windowsScriptSecretNamespace, err)
		}
		return "", fmt.Errorf("error getting Windows script data secret %q in namespace %q: %v", windowsScriptSecretName, windowsScriptSecretNamespace, err)
	}

	data, exists := scriptSecret.Data[wmcoSecretDataKey]
	if !exists {
		return "", machinecontroller.InvalidMachineConfiguration("secret %v/%v does not have %q field set. Thus, no user data applied when creating an instance", windowsScriptSecretNamespace, windowsScriptSecretName, wmcoSecretDataKey)
	}

	script := windows.RemovePowershellTags(string(data))
	return script, nil
}
